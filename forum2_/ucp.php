...
switch ($mode)
{
	...
	case 'register':
		...
		header('Location: '.EE_HTTP_PREFIX.'UA/Forma-rejestraciji.html');
		exit;
	...
	case 'login':
		...
		$_POST['action'] = 'access_package.authorize';
		$_POST['login'] = $_POST['username'];
		$_POST['passw'] = $_POST['password'];
		ap_process_login_form(false);
		if (0 == count($error))
		{
			include('ee_login.php');
		}
