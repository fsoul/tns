 ...
 */
	...
	check_modul_rights(array(ADMINISTRATOR), ADMIN_MENU_ITEM_OBJECT_FIELD);
...
//	$hidden = array('is_default');
	...
	$size_filter['id'] = 3;
	$size_filter['object_id'] = 3;
	$size_filter['one_for_all_languages'] = 3;
	// тип фильтра
	$type_filter['object'] = 'select_distinct';
	$type_filter['object_field_type'] = 'select_distinct';
	// выравнивание
	$align['id']='right';
	// стиль столбца
	...
	$mandatory=array('object_id','object_field_name','object_field_type');
	// тип поля ввода
	$type['id'] = "string";
	$type['object_id'] = "select_object";
	$type['object_field_type'] = "select_object_field_type";
	$type['one_for_all_languages'] = "checkbox";

	//$check_pattern['object_id'] = array('^[0-9]*$', 'Illegal characters in page name');
	$check_pattern['object_field_name'] = array('^[a-zA-Z_][a-zA-Z0-9_-]*$', 'Illegal characters in name');
	...
	function save()
	{
		...
		if (post('refresh'))
		{
			if (count($error)==0)
			{
				...
				if ($res == -1)
				{
					$error['object_field_name'] = 'Such object field allready exists';
				}
