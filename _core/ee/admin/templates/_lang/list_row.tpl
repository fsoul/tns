...
<tr <%tr_bgcolor%>>
	...
	<td align="center" height="25" class="table_data" nowrap="1">
		...
		<%include:<%iif::modul,_tpl_page,<%:modul%>/,,_media,<%:modul%>/,_tpl_file,%><%iif::modul,browser,,list_row_edit<%iif::modul,_news_letters,,_popup%>_btn%>%>
		<%include:<%iif::modul,_tpl_page,list_row_copy_btn%>%>
	</td>
	<td align="center">
		<%include:<%iif:<%:modul%>,_tpl_page,<%:modul%>/,_tpl_folder,<%:modul%>/,_media,<%:modul%>/%><%iif::modul,browser,,_survey,,_error_page,,<%:modul%>/list_row_del_btn%>%>
	</td>
	<td>
