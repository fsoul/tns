...
<input type="hidden" name="action" value="newsletter_slave_unsubscribe"/>
...
<input type="hidden" name="slave_unsubscribe_page" value="<%_get:slave_unsubscribe_page%>"/>
<!--input type="hidden" name="language" value="<%iif:<%:language%>,,<%_get:language%>,<%:language%>%>"/-->
<!--input type="hidden" name="t" value="<%:t%>"/-->
	<table cellpadding="0" cellspacing="0" border="0" style="text-align: left;" class="subscribe_table">
		<%iif:<%_get:result%>,not_subscribed,<tr><td colspan="2" style="color:#FF0000;"><%cms:not_subscribed%></td><td>&nbsp;</td></tr>%>
		<tr><td colspan=3>
		...
		<tr><td colspan="2"><%text_edit_page_cms:unsubscribe_to_all%><%iif:<%getField:SELECT COUNT(*) FROM v_nl_group WHERE show_on_front=1%>,0,,<input type="checkbox" name="option_1" value="all" onclick="javascript:subscribe_to_all();"> <%page_cms:unsubscribe_to_all%>%><td>&nbsp;</td></tr>
		<!--tr><td><input type="radio" name="option_1" value="current"  onclick="javascript:subscribe_to_current();"></td><td><%e_page_cms:unsubscribe_to_current%></td></tr>
		<tr><td><input type="radio" name="option_1" value="below" onclick="javascript:subscribe_below();" checked></td><td><%e_page_cms:unsubscribe_below%></td></tr-->
		<%parse_sql_to_html:SELECT * FROM v_nl_group WHERE show_on_front=1,templates/<%:MASTER_NL_TEMPLATES_FOLDER%>nl_groups_row%>

		<tr><td colspan=2 align="center"><input type="submit" size="5" value="<%page_cms:unsubscribe_text%>" class="red_button" style="width: 72px; height: 19px; line-height: 19px; background-color: #43534A; color: #FFF; font-weight: bold; font-family: Tahoma, Helvetica, Sans Serif, sans-serif; font-size: 10px; margin-top: 20px;" /><%text_edit_page_cms:unsubscribe_text%></td><td>&nbsp;</td></tr>
	</table>
