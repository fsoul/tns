...
if(!defined('EE_CAPTCHA_FONT_FILE')) define('EE_CAPTCHA_FONT_FILE',  EE_CORE_PATH."css/arial.ttf");
...
class Securimage {
  ...
  /**
   * True to use a db word list instead of a random code
   *
   * @var bool
   */
  var $use_list_from_db  = true;
  /**
   ...
   */
  function check($code, $session_clean = true)
  {
//vdump($code, '$code');
//vdump($session_clean, '$session_clean');
    $this->code_entered = md5(strtolower(str_replace(' ' ,'', $code)));
//vdump($this->code_entered, '$this->code_entered');
    $this->validate($session_clean);
//vdump($this->correct_code, '$this->correct_code');
...
*/
    if ($this->use_list_from_db) {
      $this->code = $this->readCodeFromDB();
    }
  ...
  /**
   * Reads a word list from DB
   * Added by 2K Group (dmitryg)
   * @access private
   * @since 1.0.2
   * @return mixed  false on failure, a word on success
   */
  function readCodeFromDB()
  {
...
*/
   ...
   *
   */
  function validate($session_clean = true)
  { 
    if ( isset($_SESSION['securimage_code_value']) && !empty($_SESSION['securimage_code_value']) ) {
      if (  md5(strtolower(str_replace(' ' ,'', $_SESSION['securimage_code_value']))) == $this->code_entered ) {
